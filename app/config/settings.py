...
class Settings(BaseSettings):
    bank_name: str = Field(default="bbva", alias="BANK_NAME")
    start_url: str = Field(default="https://www.bbva.com.co/", alias="START_URL")
    ...
    collection_name: str = Field(default="bank_knowledge", alias="COLLECTION_NAME")
    chunk_size: int = Field(default=900, alias="CHUNK_SIZE")
    chunk_overlap: int = Field(default=150, alias="CHUNK_OVERLAP")

    # Retrieval configuration
    top_k: int = Field(default=8, alias="TOP_K")
    final_k: int = Field(default=4, alias="FINAL_K")
    ...
    # Gemini / LangChain generation configuration
    ...
    max_new_tokens: int = Field(default=500, alias="MAX_NEW_TOKENS")

    n_history_messages: int = Field(default=6, alias="N_HISTORY_MESSAGES")
    sqlite_path: str = Field(default="data/conversations.db", alias="SQLITE_PATH")

    raw_data_path: str = Field(default="data/raw/pages.jsonl", alias="RAW_DATA_PATH")
    processed_data_path: str = Field(default="data/processed/chunks.jsonl", alias="PROCESSED_DATA_PATH")
